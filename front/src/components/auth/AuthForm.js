...
const textMap = {
  ...
  const AuthForm = ({ type }) => {
    ...
    const handleSubmit = async (e) => {
      ...
      try {
        if (type === 'signup') {
          ...
          // 회원가입 요청 보내기
          const response = await axios.post('/api1/user/signup', {
            userName : name,
          ...
          try {
          // 로그인 요청 보내기
          const response = await axios.post('/api1/user/login', {
            userEmail : email,
            userPassword : password,
          });
          ...
          try {
            // 회원정보 수정 요청 보내기
            const response = await axios.patch('/api1/user/modify', {
              userName: name,
              userPassword: password,
              userProfile: null,
            }, {
    ...
    const emailSubmit = async () => {
      ...
      try {
        // e메일 인증보내기
          const response = await axios.post('/api1/user/email', {
            email : email,
          });
          console.log(response.data)
    ...
    const emailCheck = async () => {
      ...
      try {
        // e메일 검증
          const response = await axios.post('/api1/user/email/auth', {
            authCode : authCode, 
            email : email,
          });
          console.log(response.data)
